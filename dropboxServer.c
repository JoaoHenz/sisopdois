...
int main(int argc,char *argv[]){
	...
	if (argc<6){
		printf("Escreva no formato: ./dropboxServer <endereço_do_server_1> <endereço_do_server_2> <endereço_do_server_3> <id_deste_server> <id_do_server_primario>\n\n");
		return 0;
	}
		...
		if (!recvfrom(main_socket, (char *) &login_request, PACKETSIZE, 0, (struct sockaddr *) &client, (socklen_t *) &client_len)){
			...
			if(login_request.opcode == LOGIN){
				...
				if(primary_server_id == local_server_id){
					...
					memcpy(&client,&logindata.adress,sizeof(struct sockaddr));
				}
